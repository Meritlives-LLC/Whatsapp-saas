...
export default function Subscription() {
  ...
  const usagePercent = sub?.limits?.aiRepliesPerMonth
    ? Math.min(100, Math.round((sub.usage?.aiRepliesCount / sub.limits.aiRepliesPerMonth) * 100))
    : 0;
            ...
            <div className="relative w-28 h-28 mx-auto mb-4">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                ...
                <circle cx="50" cy="50" r="40" fill="none"
                  ...
                  strokeWidth="10"
                  strokeDasharray={`${2.51327 * Math.min(usagePercent, 100)} 251.327`}
                  strokeLinecap="round"
                  style={{ transition: 'stroke-dasharray 0.5s ease' }}
                />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-xl font-bold text-gray-900">{usagePercent}%</span>
      ...
      <div className="mt-8 md:mt-10 bg-white rounded-xl border border-gray-100 shadow-sm p-5 md:p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          {[
            ...
            { q: 'Does unused AI replies roll over?', a: 'No. The counter resets on the 1st of each billing cycle.' },
          ].map(({ q, a }) => (
            <div key={q}>
