...
export default function Conversations() {
  ...
  // Mobile: show chat panel when a convo is selected
  ...
  return (
    <div className="flex h-full relative">
      ...
      <div className={`
        ...
        w-full md:w-80 bg-white border-r border-gray-100 flex-col
      `}>
      ...
      {/* ── CHAT AREA ── */}
      {/* On mobile: full screen when showChat. On desktop: flex-1 */}
      <div className={`
      ...
      `}>
        {selected ? (
          <>
            {/* Header */}
            <div className="bg-white px-4 md:px-6 py-4 border-b border-gray-100 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-900 truncate">{selected.customerName || 'Unknown'}</p>
              ...
              <div className="flex gap-1.5 flex-shrink-0">
                {['open', 'pending', 'closed'].map(s => (
                  <button key={s}
                    ...
                    className={`text-xs px-2 md:px-3 py-1 rounded-full border transition-colors ${
                ...
                <div key={i} className={`flex ${msg.direction === 'outbound' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div className={`max-w-[75%] md:max-w-md px-4 py-2.5 rounded-2xl text-sm ${
                    ...
                    <div className={`flex items-center justify-end gap-1 mt-1 ${msg.direction === 'outbound' ? 'text-green-100' : 'text-gray-400'}`}>
                      ...
                      {msg.direction === 'outbound' && (
                        msg.sentBy === 'ai' ? <Bot size={10} /> : <CheckCheck size={10} />
                      )}
                    </div>
                  </div>
                </div>
            ...
            {/* Input */}
            <div className="bg-white border-t border-gray-100 p-3 md:p-4 flex gap-3">
              <input
                value={reply}
              ...
              <button
                ...
                className="w-10 h-10 bg-green-500 hover:bg-green-600 rounded-xl flex items-center justify-center transition-colors disabled:opacity-50 flex-shrink-0">
                <Send size={16} className="text-white" />
              </button>
            </div>
          </>
        ) : (
          /* Desktop empty state — mobile never shows this since panel is hidden */
          <div className="hidden md:flex flex-1 items-center justify-center bg-gray-50">
            <div className="text-center">
              <div className="w-16 h-16 bg-green-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
                <MessageSquareIcon size={28} className="text-green-400" />
              </div>
